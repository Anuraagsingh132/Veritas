...
2. **Case 1 (Corroboration Across Documents):** Delhivery FY24 Consolidated Revenue affirmed across independent statutory and presentation documents with automatic Million-to-Crore unit normalization.
...
- **My Engineered Mitigation:**
  > *I integrated PyMuPDF's spatial bounding engine (`page.find_tables()`). It segments coordinate cell rectangles, extracts headers hierarchically, and formats table blocks into explicit, column-delimited Markdown tables before prompting the extraction engine. This ensures strict column-to-header preservation.*
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/Anuraagsingh132/Veritas.git
   cd Veritas
   ```
...
3. **(Optional) Configure Groq API Key:**
   If you want to test live cloud LLM extraction on newly uploaded custom documents, create a `.env` file in `backend/`:
   ```bash
   cp backend/.env.example backend/.env
   # Add your key: GROQ_API_KEY=gsk_...
   ```
   *(Note: You can also paste your Groq API key directly into the UI via the "API Key" modal in the top-right corner; it stays in memory and is never committed to disk).*
...
### Ingesting New PDFs & Running My Test Suite
...
#### Running My Automated Test Battery (13 Suites)
I wrote 13 targeted test suites covering unit conversions, slide layout extraction, file signature validation, prompt injection defense, entity isolation, and domain generalization:
```bash
cd backend
...
The assignment emphasized: *"The documents should guide what counts as a fact and how it is represented. Your schema, storage, interface, and output format are entirely up to you."*

Rather than enforcing a rigid, predefined schema or domain-specific columns (which would break on non-financial PDFs), I modeled facts as flexible epistemic tuples guided purely by the source documents:
...
- **Verbatim Grounding & Anti-Hallucination:** Every extracted fact requires an `exact_quote` that must exist as a verbatim substring within the source PDF page text stream. If an LLM fabricates or paraphrases a quote, my strict grounding guard rejects the fact immediately.
- **Spatial Bounding Geometry:** PyMuPDF coordinates `[x0, y0, x1, y1]` track the physical location on the page canvas, allowing the UI to visually highlight source evidence side-by-side.

---

### Epistemological Reconciliation (Beyond Simple Graph Edges)

The assignment noted: *"A graph database or visualization alone is not the solution. The interesting part is how facts are discovered, grounded, compared, and explained."*

I avoided treating relationships as simple directed edges or cosmetic node graphs. Instead, I designed my reconciliation pipeline around three protective stages:
...
### Key Decisions & Trade-Offs I Made

| Architectural Decision | My Chosen Approach | Alternative Considered | Why I Chose This |
|:---|:---|:---|:---|
| **Knowledge Store** | **SQLite with ACID WAL Mode** | Neo4j / Graph Database | The prompt explicitly noted *"a graph database alone is not the solution"*. I chose SQLite with WAL mode because it provides zero-setup, instant cross-platform reproducibility, strict relational foreign-key integrity, and sub-millisecond querying without forcing evaluators to run a Docker daemon. |
...
| **Text Chunking for Slides** | **Paragraph & Double-Newline Splitting (`\n{2,}`)** | Sentence Punctuation Splitting (`.!?`) | Presentation slide decks (e.g. Delhivery Q4 presentation) lack terminal periods. Sentence splitting dropped entire metric cards; my newline-aware chunking captures all slide metrics cleanly. |
...
1. **Large PDFs Without Performance Issues:**
   - I implemented streaming page extraction with PyMuPDF instead of loading multi-hundred-megabyte PDFs entirely into memory. Even 100+ page reports parse and index in under 5 seconds.
2. **Many PDFs in the Same Knowledge Layer:**
   - I structured the database to concurrently index both the *Delhivery Logistics* and *India Macroeconomy* corpuses (6 starter documents, 240+ grounded facts, 50+ cross-document relationships) with strict document isolation and zero cross-contamination.
3. **Schema That Evolves Dynamically:**
   - I designed the extraction schema to dynamically infer new fact categories, predicates, and units on the fly. I verified this against unseen scientific domains (e.g. marine biology / emperor penguins in `test_generalization.py`) with zero code alterations.
4. **Incremental Ingestion Without Rebuilding:**
   - When a user uploads a new PDF, Veritas commits the document and reconciles *only the newly extracted facts* against existing knowledge base facts, avoiding $O(N^2)$ full rebuilds.

---

## ⚠️ Limitations & What I Would Build Next

### Current Limitations
1. **Scanned / Raster-Only PDFs:**
   - My current ingestion engine relies on digital PDF text streams. Scanned image PDFs without an OCR layer will yield empty character streams unless an OCR pre-processor (like Tesseract) is added.
...
3. **Real-Time Cross-Currency Exchange Rates:**
   - My unit normalization handles Indian numbering scales (Crores, Lakhs, Millions, Billions) and percentages. It does not currently fetch real-time Forex exchange rates to convert across international currencies (e.g. USD to INR).

### What I Would Build Next (Roadmap)
If I had more time or was deploying Veritas into a full production cluster, here is what I would build next:
